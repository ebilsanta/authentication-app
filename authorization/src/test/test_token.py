...
def test_token_with_ath_ok():
    ...
    dpop = create_dpop_jwt(pvk, pbk, sets.authz_url, 'POST', 
                           base64.b64encode(
                               hashlib.sha256(
                                   access_token.encode('utf-8')).digest()).decode())[1:-1]
    ...
    assert not err
    assert res
    assert(dpop_decoded['ath'] == base64.b64encode(
        hashlib.sha256(access_token.encode('utf-8')).digest()).decode())
...
def test_refresh_ok():
    ...
    payload = {
        ...
        "exp": now + 86400,                                                 
        "iat": now,
        "cnf.jkt": base64.b64encode(hashlib.sha256(sets.authz_pub_key.encode('utf-8')).digest()).decode(),
        "typ": "dpop+refresh"
    }
    ...
    dpop = create_dpop_jwt(pvk, pbk, sets.authz_url, 'POST', 
                        base64.b64encode(
                            hashlib.sha256(
                                refresh_token.encode('utf-8')).digest()).decode())[1:-1]
